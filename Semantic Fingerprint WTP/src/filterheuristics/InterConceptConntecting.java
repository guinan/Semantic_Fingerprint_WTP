...
public class InterConceptConntecting {
	...
	public void filterBestN(WTPGraph graph, List<Path> paths, int maxNumber, Map<String, String> correspondingKeywords){
		generateCountingIncludingPathMap(paths);
	...
	public void filter(WTPGraph graph, LinkedList<Path> paths,
			Map<String, String> correspondingKeywords) {
		tidyPaths(paths, correspondingKeywords);
		justKeepPathNodes(paths, graph);

		
	}
	
	private void justKeepPathNodes(LinkedList<Path> paths, WTPGraph graph) {
		...
		System.out.println("jopjopjop   "+graph.getGraph().getNodeCount() );
		List<Node> toDelete = new LinkedList<Node>();
		for(Node n : graph.getGraph().getNodeSet()){
			if(!keepers.contains(n.getId()))
				toDelete.add(n);
		}
		graph.getGraph().getNodeSet().removeAll(toDelete);
		System.out.println("jopjopjop   "+graph.getGraph().getNodeCount() );
